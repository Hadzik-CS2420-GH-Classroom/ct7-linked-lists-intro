...
//   - We could just use Node* directly in main, but the caller would have to
//     manually manage every pointer, track the size, and remember to delete everything
//   - The class handles all of that — the user just calls push_front(), pop_front(), etc.
//   - Same idea behind std::vector, std::list, and every other STL container
...
public:
    ...
    //   - We defined a destructor, so we must handle copies too — here we 'delete' them
    //   - Without this, the compiler auto-generates a shallow copy: two lists pointing
    //     to the SAME nodes → one destructor fires → dangling pointers → crash
    //
    // ! DISCUSSION: "Do we have to delete move too?"
    //   - Technically no — the compiler won't auto-generate move operations
    //     when you've declared copy operations
    //   - But explicitly deleting all four makes the intent crystal clear
...
private:
    ...
    //   - These are default member initializers (C++11)
    //   - They guarantee head_ and size_ start with sane values even
    //     if we forget to set them in a constructor
    //   - Without this, head_ would contain garbage — a pointer to random memory
    Node* head_ = nullptr;
    int size_ = 0;
};
